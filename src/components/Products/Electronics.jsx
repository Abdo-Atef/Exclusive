...
export default function Electronics() {
  ...
  let {getProductsByCategory, checkRating} = useContext(ProductsContext);
  ...
  async function getElectronicsProducts() {
    let ElectronicsId = '6439d2d167d9aa4ca970649f';
    let {data} =  await getProductsByCategory(page, 50, ElectronicsId)
    return data
  }
  ...
  let {data, isLoading} = useQuery('Electronics', getElectronicsProducts)
      ...
      <div key={index}>
        <div className='productCard mx-2 border rounded-2 d-block text-start'>
          <div className=' position-relative'>
            ...
            <button onClick={()=>dispatch(addToCart(product._id))} className='addToCart btn text1 rounded-bottom-0 w-100 main-bg-black position-absolute'>Add To Cart </button>
